...
import { deleteFaculty } from "@/app/actions";
...
export default function DeleteFacultyButton({
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteFaculty(facultyId, userId);

      if (result.success) {
        toast.success("Faculty deleted");
        router.refresh()
      } else if (result.error === DeleteErrorCode.HAS_DEPENDENCIES) {
        toast.error("Cannot delete: Faculty is assigned to course. Free the faculty first.");
      } else {
